package com.confluent.window;

import com.confluent.StreamsUtils;
import com.confluent.time.OrderTimestampProducer;
...
public class WindowProducer<T>  implements Closeable {
    ...
    public static void main() {
        ...
        try( WindowProducer<ElectronicOrder> aggregateProducer = new WindowProducer()){

            aggregateProducer.sendRecords(
                    "windowed-input-topic",
    ...
    private  void sendRecords(
        ...
        records.forEach(record -> {
            ...
            ProducerRecord<String, T> producerRecord =
                    new ProducerRecord<>(topic, 0, timeExtractor.apply(record),  keyExtractor.apply(record).toString(), record);
            try {
