...
import com.confluent.aggregagte.AggregateProducer;
...
public class OrderTimestampProducer<T> implements Closeable {

    private KafkaProducer<String, T>  producer ;

    public static  final  String inputTopic = "extractor.input.topic";


    public  OrderTimestampProducer(){
        this.producer = new KafkaProducer<>(producerProps(KafkaAvroSerializer.class.getName()));
    }

    public static void main() {

        var electronicOrdersListToPublish = createScenarioData();

        try( OrderTimestampProducer<ElectronicOrder> aggregateProducer = new OrderTimestampProducer()){

            aggregateProducer.sendRecords(
                    "processor-input-topic",
                    electronicOrdersListToPublish,
    ...
    private  void sendRecords(
        ...
        records.forEach(record -> {
            ...
            ProducerRecord<String, T> producerRecord =
                    new ProducerRecord<>(topic, 0, keyExtractor.apply(record).toString(), record);
            try {
                RecordMetadata metadata = producer.send(producerRecord,  StreamsUtils.callback()).get();
    ...
    private static List<ElectronicOrder> createScenarioData() {
      ...
      return List.of(electronicOrderOne, electronicOrderTwo, electronicOrderThree,electronicOrderFour, electronicOrderFive );
